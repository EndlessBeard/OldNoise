...
public class PerlinNoise
{
	...
	public float FractalNoiseControl(float x, float y, float z, int octNum, float frq, float amp, float xn, float yn, float zn)
	{
		float gain = 1.0f;
		float sum = 0.0f;
		...
		for(int i = 0; i < octNum; i++)
		{
			sum +=  Noise3D(xn*x*gain/frq, yn*y*gain/frq, zn*z*gain/frq) * amp/gain;
			gain *= 2.0f;
		}
		return sum;
	}
